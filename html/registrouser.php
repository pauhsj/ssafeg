...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Sanitizar teléfono (eliminar caracteres no numéricos y limitar longitud)
...
<head>
  ...
  <style>
    * { box-sizing: border-box; font-family: 'Segoe UI', sans-serif; margin: 0; padding: 0; }
    ...
    .container { display: flex; flex-direction: column; min-height: 100vh; padding: 20px; justify-content: center; align-items: center; }
    .card {
      background: #fff; border-radius: 12px; box-shadow: 0 0 15px rgba(0,0,0,0.1);
    ...
    .left {
      background-color: #dcedc8; flex: 1;
      display: flex; flex-direction: column; justify-content: center; align-items: center; padding: 30px; text-align: center;
    }
    .left img {
      width: 100%; max-width: 300px; border-radius: 10px; margin-bottom: 20px;
    }
    .left h1 { font-size: 26px; color: #33691e; }
    .right { flex: 1; padding: 40px; }
    .form h2 {
      font-size: 24px; color: #2e7d32; margin-bottom: 10px; text-align: center;
    }
    .form p { color: #666; text-align: center; margin-bottom: 20px; font-size: 14px; }
    .form input {
      width: 100%; padding: 12px; margin-bottom: 15px; border: 1px solid #ccc; border-radius: 6px;
    }
    .form button {
      width: 100%; padding: 12px; background-color: #388e3c; color: #fff; border: none; border-radius: 6px;
...
<div style="position: relative;">
  <input type="password" name="contraseña" id="contraseña" placeholder="Contraseña" required style="padding-right: 40px;" />
  <i class="fa fa-eye" id="togglePassword" style="position:absolute; right:10px; top:13px; cursor:pointer; color:gray;"></i>
</div>
  ...
  document.getElementById('registerForm').addEventListener('submit', function(e) {
    ...
    if (!passwordRegex.test(contraseña)) {
      ...
      Swal.fire({
        ...
        html: 'La contraseña debe tener al menos:<br>• 8 caracteres<br>• 1 letra<br>• 1 número',
        confirmButtonColor: '#388e3c'
      });
      return;
    }
